...
//    test2(args);
  ...
  static double[] averagexy = new double[] {
      5, 8};
    ...
    for (int rr = 0; rr < (r != 0 ? 2 : 1); rr++) {
      for (int gg = 0; gg < (g != 0 ? 2 : 1); gg++) {
        for (int bb = 0; bb < (b != 0 ? 2 : 1); bb++) {
          ...
          double dist = Math.sqrt(Maths.sqr(averagexy[0] * xy[0]) +
                                  Maths.sqr(averagexy[1] * xy[1]));
  ...
  public static void test3(String[] args) {
    ...
    int nullcount = 0; //1&3象限null最少
    double totaldist = 0;
    for (int r = 0; r <= 3; r++) {
      for (int g = 0; g <= 3; g++) {
        for (int b = 0; b <= 3; b++) {
          int[] bestRGB = getBestRGB(r, g, b, 3);
//          System.out.println(Arrays.toString(bestRGB) + " " + mindist + " \t" +
//                             Arrays.toString(minxy));
          System.out.println(r + " " + g + " " + b + " " +
                             Arrays.toString(bestRGB) + " \t" +
                             Arrays.toString(minxy) + " " +
                             (minxy != null ? getQuadrant(minxy[0], minxy[1]) :
                              ""));
          if (null == bestRGB) {
            nullcount++;
          }
