  ...
  public static void processDot(String[] args) {
    ...
    try {
      BufferedReader br = new BufferedReader(new FileReader(
          "D:/ณnล้/nobody zone/exp data/0914/test.csv"));
...
//    double[] dataDoubleArray = (double[]) list.toArray(new double[list.size()]);
    Plot2D plot = Plot2D.getInstance();
    int size = list.size();
    //double[] data = new double[size];
    for (int x = 0; x < size; x++) {
      double d = list.get(x).doubleValue();
      //plot.addLinePlot("", 0, 1, data);
//      plot.addCacheLinePlot("", 0, 1, d);
