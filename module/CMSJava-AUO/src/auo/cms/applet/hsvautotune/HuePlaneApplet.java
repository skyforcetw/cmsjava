...
//    try {
//      txt2 = this.getParameter("param1", "hsvip.txt");
//    }
//    catch (Exception e) {
//      e.printStackTrace();
//    }
...
//    ( (PlotPanel) panel).removePlotToolBar();
  ...
  public void start() {
    URL base = null;
    try {
      base = this.isActive() ? this.getDocumentBase() :
    ...
    try {
      URL url = new URL(base,
                        "hueplane/" + dir + "/new.txt");
      newDataArray = DoubleArray.to3DDoubleArray(ASCIIFile.readDoubleArray(url.
          openStream()), 10, 10);

      url = new URL(base,
                    "hueplane/" + dir + "/target.txt");
      targetDataArray = DoubleArray.to3DDoubleArray(ASCIIFile.readDoubleArray(
          url.openStream()), 10, 10);
