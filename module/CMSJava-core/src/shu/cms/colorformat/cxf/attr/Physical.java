...
public class Physical
  ...
  public final static String FILTER_UNKNOWN = "Unknown";
  public final static String SPECTRUM_TYPE_EMISSION = "Emission";
  ...
  protected final static String[] names = new String[] {
      "Physical.Filter",
      "Physical.NumberOfDataPointsSpectrum",
      "Physical.SpectrumType",
      "Physical.WavelengthSpectrumMax",
      "Physical.WavelengthSpectrumMin"
  };

  public static void main(String[] args) {
    Physical p = new Physical();
    System.out.println(p.getClass().getName());
//    System.out.println( p.Filter.getClass().getFields()[0].getName());
  }
  ...
  public static Physical getInstance(List<Attribute> attrs) {
    ...
    for (Attribute attr : attrs) {
      if (attr.getName().equals(names[0])) {
        ...
        physical.wavelengthSpectrumMax = Integer.parseInt(attr.getvalue().
            replaceAll("nm", "").trim());
      }
      else if (attr.getName().equals(names[4])) {
        physical.wavelengthSpectrumMin = Integer.parseInt(attr.getvalue().
            replaceAll("nm", "").trim());
      }
    }
    return physical;
