...
public class CxFTransformer {
   ...
   * @return Sample
   */
  private static Sample spectraWinToSample(SpectraWinAsciiFile file,
                                           String sampleName) {
    if (file == null) {
  ...
  private static Conditions spectraWinToConditions(SpectraWinAsciiFile file) {
    ...
    String num = Integer.toString( (spectral.end - spectral.start) /
                                  spectral.interval + 1);
  ...
  public static void main(String[] args) {
    GretagMacbethAsciiParser parser = new GretagMacbethAsciiParser(
        "Reference Files\\Monitor\\LCD Monitor Reference 742.txt");
    GretagMacbethAsciiFile file = parser.getGretagMacbethAsciiFile();
    CXF cxf = RGBICCProfileToCxF(file);
    CXFOperator.saveCXF(cxf,
                        "Reference Files\\Monitor\\LCD Monitor Reference 742.cxf");
  }
  ...
  private static Conditions PDDEToConditions(PrepressDigitalDataExchangeFile
                                             file) {
    Conditions conditions = objFactory.createConditions();
    conditions.setID("1");
    List<Attribute> attrs = conditions.getAttribute();
  ...
  private static SampleSet RGBICCProfileToSampleSet(GretagMacbethAsciiFile file) {
    ...
    for (int x = 0; x < size; x++) {
      GretagMacbethAsciiFile.TestChartData testChartData = dataSet.
          getTestChartData(x);
      Sample sample = objFactory.createSample();
