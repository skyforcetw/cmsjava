 ...
 * @version 1.0
 */
public class LCDTargetNumberAdapter
  ...
  private final static List<RGB> getRGBList(Number number) {
...
//      case Camera:
//        return getCameraRGBList(number);
      default:
        return null;
    }
  ...
  private final static List<RGB> getComplexRGBList(Number number) {
    if (!number.isComplex()) {
...
//  private final static List<RGB> getCameraRGBList(Number number) {
//    switch (number) {
//      case ColorChecker24:
//        return null;
//      default:
...
//  }
  ...
  private final static List<RGB> getUnknowRGBList(Number number) {
    switch (number) {
      case Surface1352:
        return LCDTargetBase.SurfaceTarget.getSurface( (int) number.getStep());
      case Patch218:
        return LCDTarget.Instance.get(LCDTarget.Number.Test729).
            targetFilter.getPatch218From729().filter.rgbList();
//      case ColorCheck24:
//        return null;
      default:
        return null;
  ...
  private final static List<RGB> getRampRGBList(Number number) {
    if (!number.isRamp()) {
  ...
  private final static class TestTarget {

    private final static List<RGB> getAUOTest(int step) {
    ...
    private final static List<RGB> getTest(int start, int step,
                                           boolean autoTail) {
    ...
    private final static List<RGB> getTest(int step) {
      return getTest(0, step, false);
    }

  }

  private final static class XTalkTarget {

    private final static List<RGB> getXTalk0(int start, int step,
                                             RGBBase.Channel[][] channelsSet) {
      int level = (int) Math.rint( (256. - start) / step);
      level = (start + step * (level - 1) < 255) ? level + 1 : level;
      int size = level * level * 3 + 1; //add 1 for white
      List<RGB> rgbList = new ArrayList<RGB> (size);
    ...
    private final static RGBBase.Channel[][] getXTalkChannelsSet() {
      RGBBase.Channel[][] channelsSet = new RGBBase.Channel[3][];
      for (int x = 0; x < 3; x++) {
        RGBBase.Channel ch0 = RGBBase.Channel.getChannelByArrayIndex(x);
        RGBBase.Channel ch1 = RGBBase.Channel.getChannelByArrayIndex( (x + 1) %
            3);

        if (ch0.getArrayIndex() < ch1.getArrayIndex()) {
          channelsSet[x] = new RGBBase.Channel[] {
              ch0, ch1};
        }
        else {
          channelsSet[x] = new RGBBase.Channel[] {
    ...
    private final static List<RGB> getXTalk(int start, int step) {
      RGBBase.Channel[][] channelsSet = getXTalkChannelsSet();
      return getXTalk0(start, step, channelsSet);
    }

    private final static List<RGB> getXTalk(int step) {
      RGBBase.Channel[][] channelsSet = getXTalkChannelsSet();
      return getXTalk0(step, step, channelsSet);
    }
  }

  private final static class RampTarget {
    private final static List<RGB> getRamp(RGBBase.Channel ch,
                                           boolean withWhite) {
      List<RGB>
          rgbList = getRamp(new RGBBase.Channel[] {ch}, withWhite, 1,
    ...
    private final static List<RGB> getRamp(RGBBase.Channel[] channels,
                                           boolean sizePlusOne, int step,
    ...
    private final static List<RGB> getRamp1024() {
      return getRamp(RGBBase.Channel.WRGBChannel, false, 1,
                     RGB.MaxValue.Int8Bit);
    }

    private final static List<RGB> getRamp897() {
    ...
    private final static List<RGB> getRamp1021() {
    ...
    private final static List<RGB> getRamp256RGB_W() {
    ...
    private final static List<RGB> getRamp1792() {
      return getRamp(RGBBase.Channel.RGBYMCWChannel, false, 1,
                     RGB.MaxValue.Int8Bit);
    }

    private final static List<RGB> getRamp257() {
      List<RGB>
          rgbList = getRamp(RGBBase.Channel.RGBWChannel, true, 4,
                            RGB.MaxValue.Int6Bit);
      RGB white = (RGB) White.clone();
      rgbList.add(white);
      return rgbList;
    }

    private final static List<RGB> getRamp256() {
      List<RGB>
          rgbList = getRamp(RGBBase.Channel.WRGBChannel, false, 4,
                            RGB.MaxValue.Int6Bit);
      return rgbList;
    }

    private final static List<RGB> getRamp260() {
      List<RGB> rgbList = new ArrayList<RGB> (260);
      RGB.Channel channels[] = new RGB.Channel[] {
          RGB.Channel.W, RGB.Channel.R, RGB.Channel.G, RGB.Channel.B};
      for (RGBBase.Channel ch : channels) {
  ...
  private final static List<RGB> getTestRGBList(Number number) {
    if (!number.isTest()) {
  ...
  private final static List<RGB> getXTalkRGBList(Number number) {
    if (!number.isXTalk()) {
  ...
  public static void main(String[] args) {
    LCDTargetNumberAdapter adapter = new LCDTargetNumberAdapter(LCDTargetBase.
        Number.Test729);
