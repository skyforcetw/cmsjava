 ...
 *
 * <p>Description: </p>
 * LCD導表,內部儲存XYZRGB的Patch
 * 支援i1pro/spectrolino以光譜能量分佈值的所儲存的格式產生
 * 支援i1display2以Lab所儲存的格式產生
 *
...
public final class LCDTarget
  ...
  public double getStep() {
    ...
    switch (number) {
      case Ramp257_6Bit:
      case Ramp260:
//        return (4. / 255) * this.getMaxValue().max;
      case Ramp509:
      case Ramp896:
      case Ramp897:
//        return (2. / 255) * this.getMaxValue().max;
      case Ramp256W:
      case Ramp256G:
      case Ramp256R_W:
      case Ramp256G_W:
...
//        return (getCodeBit().getStep() / 255) * this.getMaxValue().max;
      default:
        return -1;
    }
  }
...
//  public final void replaceRGBToLinear() {
//    Number number = this.getNumber();
//    if (number.targetType != TargetType.Ramp) {
//      throw new IllegalStateException("Number.targetType != TargetType.Ramp.");
//    }
...
//      case Ramp256G:
//      case Ramp256B:
//        rgbList = LCDTarget.Instance.get(number).filter.rgbList();
//        break;
//      default:
//        throw new IllegalStateException("Unsupported Ramp.");
//    }
//    List<Patch> patchList = this.getPatchList();
//    if (patchList.size() != rgbList.size()) {
//      throw new IllegalStateException("patchList.size() != rgbList.size().");
//    }
...
//  }
  ...
  protected LCDTarget(List<Spectra> spectraList,
      List<RGB> rgbList, Number type, boolean inverseModeMeasure) {
    this(Patch.Produce.XYZRGBPatches(spectraList, null, rgbList, defaultCMF),
         type, inverseModeMeasure);
  }

  protected LCDTarget(List<Patch> patchList, Number number,
      boolean inverseModeMeasure) {
    this(patchList, number, false, inverseModeMeasure);
  }

  protected LCDTarget(List<Patch> patchList, Number number,
      boolean isRGBPatchOnly, boolean inverseModeMeasure) {
    super(patchList);
    ...
    if (number == null) {
      ...
      if (number.getPatchCount() > 0 &&
          number.getPatchCount() != patchList.size()) {
        throw new IllegalArgumentException(
            "number.patchNumber != patchList.size()!");
      }
    }
  ...
  public Patch getSaturatedChannelPatch(RGBBase.Channel channel) {
    ...
    double max = /*getNumber().is6Bit() ? 252 :*/ rgb.getMaxValue().max; // (2^6-1)*4
  ...
  public static void main(String[] args) {
    LCDTarget target = LCDTarget.Instance.get("auo_M240HW01", Source.CA210,
                                              Number.Ramp1021,
                                              FileType.Logo, "091216-2", null);
//    System.out.println(target.hook.getHookedIndex());
//    target.hook.fixHook();
  ...
  public final class TargetFilter {
    ...
    private LCDTarget getWHQLFrom2223() {
      ...
      if (number != Number.Complex729_46_96_1352) {
        throw new IllegalStateException(
            "number != Number.Complex729_46_96_1352");
      }
    ...
    public LCDTarget getRamp896From1792() {
      ...
      for (Patch p : getPatchList()) {
        ...
        if (rgbV[0] != 0 && rgbV[0] != 254 &&
            (rgbV[0] % 2 == 0 || rgbV[0] == 255) ||
            rgbV[1] != 0 && rgbV[1] != 254 &&
            (rgbV[1] % 2 == 0 || rgbV[1] == 255) ||
            rgbV[2] != 0 && rgbV[2] != 254 &&
            (rgbV[2] % 2 == 0 || rgbV[2] == 255) ||
            rgb.isWhite() ||
            rgb.isBlack()) {
          tmp.add(p);
        }
      }
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp896,
                                          inverseModeMeasure);
    ...
    public LCDTarget getRamp257() {
      switch (getNumber()) {
        ...
        case Ramp257_6Bit:
          return this.parent;
        case Ramp260:
          return getPartLCDTarget(Number.Ramp257_6Bit, " 257 From 260", 0, 64,
                                  65, 129, 130, 194, 195, 260);
        case Ramp1021:
        case Ramp1024:
          return getRamp260From102x().targetFilter.getRamp257();
        default:
          throw new IllegalStateException("Unsupported LCDTargetBase.Number: " +
                                          getNumber());
      }
    }
    ...
    private LCDTarget getRampW256From102x() {
      if (getNumber() != Number.Ramp1024 && getNumber() != Number.Ramp1021) {
        throw new IllegalStateException(
            "getNumber() != Number.Ramp1024 & Number.Ramp1021");
      }
      String desc = getNumber() == Number.Ramp1024 ? " 256WFrom1024" :
          " 256WFrom1021";
...
//          throw new IllegalStateException(
//              "Unsupported LCDTargetBase.Number: " +
//              getNumber());
      }
    }
    ...
    private LCDTarget getRamp256From102x(RGBBase.Channel ch, boolean withWhite) {
      ...
      if (orgnumber != Number.Ramp1024 && orgnumber != Number.Ramp1021) {
        throw new IllegalStateException(
            "orgnumber != Number.Ramp1024 & Number.Ramp1021");
      }
      ...
      switch (orgnumber) {
        case Ramp1024:
          ...
          return this.getPartLCDTarget(number, desc, start, start + 256,
                                       whiteStart, whiteEnd);
        case Ramp1021:
          start = 256 * (ch.getArrayIndex() + 1) - ch.getArrayIndex() * 1;
          return this.getPartLCDTarget(number, desc, 0, 1, start, start + 255,
                                       whiteStart, whiteEnd);
        default:
          throw new IllegalStateException("Unsupported LCDTargetBase.Number: " +
                                          orgnumber);
    ...
    private LCDTarget getRamp260From102x() {
      ...
      String desc = getNumber() == Number.Ramp1024 ? " 260From1024" :
          " 260From1021";
      ...
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp260,
                                          inverseModeMeasure);
    ...
    public LCDTarget getByNumber(Number number, String newdescription) {
      ...
      LCDTarget lcdTarget = new LCDTarget(tmp, number,
                                          inverseModeMeasure);
      newdescription = (newdescription == null) ?
          " " + number + "From" + getNumber() : newdescription;
...
//      Patch.Filter.leastOneZeroAndWhite(getPatchList(), tmp);
      Patch.Filter.leastOneZero(getPatchList(), tmp, true);
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Patch62,
                                          inverseModeMeasure);
    ...
    public LCDTarget getRamp509From1021() {
      ...
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp509,
                                          inverseModeMeasure);
    ...
    public LCDTarget getRamp2048From4096() {
      ...
      return getPartLCDTarget(Number.Ramp2048, " 2048 From 4096", 0, 1024, 2048,
                              3072);
    }
    ...
    public LCDTarget getRamp3577From7147() {
      ...
      for (Patch p : getPatchList()) {
        ...
        if ( (rgb.isWhite() || rgb.isBlack()) ||
            ( (rgbValues[0] % 1 == 0 || rgbValues[0] % 1 == 0.5) &&
             (rgbValues[1] % 1 == 0 || rgbValues[1] % 1 == 0.5) &&
             (rgbValues[2] % 1 == 0 || rgbValues[2] % 1 == 0.5))) {
          tmp.add(p);
        }

      }
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp3577,
                                          inverseModeMeasure);
    ...
    public LCDTarget getPatch218From729() {
      ...
      Patch.Filter.leastOneZero(getPatchList(), tmp, true);
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Patch218, isRGBPatchOnly,
                                          inverseModeMeasure);
    ...
    private final LCDTarget getTargetFromComplex(Number targetNumber) {
      ...
      return getPartLCDTarget(targetNumber,
                              " " + targetNumber.getPatchCount() + "From" +
                              number.getPatchCount(), indexs[0], indexs[1]);
    }
    ...
    public LCDTarget getSurface1352From2223() {
      if (getNumber() != Number.Complex729_46_96_1352) {
        throw new IllegalStateException(
            "getNumber() != Number.Complex729_46_96_1352");
      }
      return getPartLCDTarget(Number.Surface1352, "Surface 1352 from 2223", 871,
                              2223);
    }
    ...
    private LCDTarget getRamp1021From1024() {
      ...
      return getPartLCDTarget(Number.Ramp1021, " 1021 From 1024", 0, 256, 257,
                              512, 514, 768, 770, 1024);
    }

    /**
     * 取得部分的patch成為新的LCDTarget
     * @param partNumber Number
     * @param newdescription String
     * @param index int[]
     * @return LCDTarget
     */
    private LCDTarget getPartLCDTarget(Number partNumber,
                                       String newdescription,
                                       int ...index) {
      List<Patch> filterPatchList = filterByIndex(index);
      LCDTarget lcdTarget = new LCDTarget(filterPatchList, partNumber,
                                          inverseModeMeasure);
      initLCDTarget(lcdTarget, newdescription);
      return lcdTarget;
    }
    ...
    public LCDTarget getRamp1792From7147() {
      ...
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp1792,
                                          inverseModeMeasure);
    ...
    public LCDTarget getRamp7147From7168() {
      ...
      LCDTarget lcdTarget = new LCDTarget(tmp, Number.Ramp7147,
                                          inverseModeMeasure);
  ...
  public final class Filter {
    ...
    /**
     * 過濾出RGB為gray的色塊(R=G=B), 且確保亮度是遞增的
     * @return List
     */
...
//      Patch.Filter.leastOneZero(getPatchList(), tmp);
...
//      boolean ramp6bit = Number.Ramp256_6Bit == getNumber() ||
//          Number.Ramp257_6Bit == getNumber();
//      double max6bit = RGB.MaxValue.Int6Bit.max;

      for (int x = 0; x < patchList.size(); x++) {
        Patch p = patchList.get(x);
        RGB rgb = p.getRGB();
//        double[] rgbValues = rgb.getValues(new double[3], RGB.MaxValue.Int6Bit);
        if (rgb.isWhite() /* ||
             (ramp6bit && rgb.R == max6bit && rgb.G == max6bit &&
                                       rgb.B == max6bit)*/) {
          whitePatchIndex = x;
          break;
        }
