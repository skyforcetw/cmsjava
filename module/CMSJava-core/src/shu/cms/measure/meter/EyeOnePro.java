...
public class EyeOnePro extends EyeOneDisplay2 {
    public EyeOnePro(ScreenType screenType) {
        super(screenType);
        try {
            _EYE_ONE.setMeasurementMode(EyeOneAPI.MeasurementMode.
                                        SingleEmission);
    ...
    public Spectra triggerMeasurementInSpectra() {
        try {
            ...
            Spectra s = new Spectra("", Spectra.SpectrumType.NO_ASSIGN, 380,
                                    730, 10,
                                    data);
            return s;
        } catch (EyeOneException ex) {
    ...
    public void setLogoFileHeader(LogoFile logo) {
        if (spectraMode) {
            ...
            logo.setDataFormat(
                    "SampleID	SAMPLE_NAME	RGB_R	RGB_G	RGB_B	XYZ_X	XYZ_Y	XYZ_Z	nm380	nm390	nm400	nm410	nm420	nm430	nm440	nm450	nm460	nm470	nm480	nm490	nm500	nm510	nm520	nm530	nm540	nm550	nm560	nm570	nm580	nm590	nm600	nm610	nm620	nm630	nm640	nm650	nm660	nm670	nm680	nm690	nm700	nm710	nm720	nm730");
        } else {
    ...
    public void setLogoFileData(LogoFile logo, List<Patch> patchList) {
        if (spectraMode) {
            ...
            for (int x = 0; x < size; x++) {
                ...
                buf.append(index + "\t" + name + "\t" + rgb.R + "\t" + rgb.G +
                           "\t" + rgb.B + "\t" + XYZ.X + "\t" + XYZ.Y + " " +
                           XYZ.Z);
