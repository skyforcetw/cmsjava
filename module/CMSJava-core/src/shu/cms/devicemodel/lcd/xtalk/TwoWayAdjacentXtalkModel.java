 ...
 * @version 1.0
 */
public class TwoWayAdjacentXtalkModel
    extends AdjacentPixelXtalkModel {
  ...
  protected Factor[] _produceFactor() {
    ...
    reconstructors = new AbstractXTalkReconstructor[] {
        leftReconstructor, rightReconstructor};

    double[][][] rCorrectLut = getTwoWayCorrectLUT(RGBBase.Channel.R,
        codeValues);
    double[][][] gCorrectLut = getTwoWayCorrectLUT(RGBBase.Channel.G,
        codeValues);
    double[][][] bCorrectLut = getTwoWayCorrectLUT(RGBBase.Channel.B,
        codeValues);

    leftEliminator = new XTalkEliminator(codeValues, codeValues, rCorrectLut[0],
                                         gCorrectLut[0], bCorrectLut[0], false);
    rightEliminator = new XTalkEliminator(codeValues, codeValues, rCorrectLut[1],
                                          gCorrectLut[1], bCorrectLut[1], false);
    eliminators = new XTalkEliminator[] {
        leftEliminator, rightEliminator};
  ...
  /**
   * 解析出selfChannel被影響的對照表.
   * 如果selfChannel為G, 則解析出R->G(selfChannel之左)以及B->G(selfChannel之右)的影響性.
   * @param selfChannel Channel
   * @param channelValues double[]
   * @return double[][][]
   */
  protected double[][][] getTwoWayCorrectLUT(RGBBase.Channel selfChannel,
                                             double[] channelValues) {
    RGBBase.Channel leftChannel = this.leftProperty.getAdjacentChannel(
        selfChannel);
    RGBBase.Channel rightChannel = this.rightProperty.getAdjacentChannel(
        selfChannel);
    ...
    double[][][] correctLut = new double[][][] {
        leftCorrectLut, rightCorrectLut};
...
//    double[][][] correctLut = eliminator.correctLut;
    ...
    for (int x = 0; x < 3; x++) {
      for (int c = 0; c < 2; c++) {
        ...
        plots[index] = Plot3D.getInstance(adjch.name() + "->" + ch.name() +
                                          " Correct LUT");
        plots[index].addGridPlot(ch.name(), ch.color, codeValues, codeValues,
                                 correctLut[x]);
  ...
  public static void main(String[] args) {
    ...
    LCDModel model = new TwoWayAdjacentXtalkModel(ramplcdTarget,
                                                  xtalklcdTarget);
...
//    model.plotModelReport(report);
  ...
  private XTalkProperty[] properties = new XTalkProperty[] {
  ...
  protected final RGB correctRGB(final RGB rgb, boolean uncorrect) {
    ...
    for (RGBBase.Channel ch : RGBBase.Channel.RGBChannel) {
      ...
      double leftCorrectValues = leftEliminator.getCorrectionValue(ch,
          selfValues, leftValues);
      double rightCorrectValues = rightEliminator.getCorrectionValue(ch,
