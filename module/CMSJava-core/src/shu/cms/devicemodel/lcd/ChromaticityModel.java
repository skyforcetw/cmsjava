 ...
 * @version 1.0
 */
public class ChromaticityModel
    extends LCDModel {
...
//      double p = SCurveModel.getPointOfSCurve(luminanceSCurveParams[index],
//                                              input / 255.);
//      double luminance = p * pureMaxLuminance[index]; // + blackXYZ.Y;
//      double ciex = regress[index][0].getPredict(new double[] {input})[0];
//      double ciey = regress[index][1].getPredict(new double[] {ciex})[0];
//      CIExyY xyY = new CIExyY(ciex, ciey, luminance);
...
//  }
  ...
  public static double[] getSmoothCurve(double[] originalCurve,
                                        double[] deltaCurve) {
    ...
    for (RGB.Channel ch : chs) {
      ...
      if (plotting) {
        ...
        for (int x = 0; x < size; x++) {
          ...
          plot.addCacheScatterLinePlot(ch.name() + "smooth", outputx[x],
                                       outputy[x]);

          if (x > 0) {
            dplot.addCacheScatterLinePlot(ch.name() + "x_", Color.black, x,
                                          outputdx[x]);
            dplot.addCacheScatterLinePlot(ch.name() + "y_", Color.black, x,
                                          outputdy[x]);
          }
          if (prexyY != null) {
            dplot.addCacheScatterLinePlot(ch.name() + "x", c, x,
                                          xyY.x - prexyY.x);
            dplot.addCacheScatterLinePlot(ch.name() + "y", c, x,
                                          xyY.y - prexyY.y);
          }
      ...
      Interpolation1DLUT xlut = new Interpolation1DLUT(intputGrayLevel,
          outputx, Interpolation1DLUT.Algo.LINEAR);
      Interpolation1DLUT ylut = new Interpolation1DLUT(intputGrayLevel,
          outputy, Interpolation1DLUT.Algo.LINEAR);
      Interpolation1DLUT Ylut = new Interpolation1DLUT(intputGrayLevel,
          outputY, Interpolation1DLUT.Algo.LINEAR);
  ...
  protected Factor[] _produceFactor_() {
    boolean plotting = true;
    CIEXYZ flareXYZ = this.flare.getFlare(); //k
    RGB.Channel[] chs = RGB.Channel.RGBChannel;
    double[][][] rgbxyYCurve = new double[3][3][];
    Plot2D plot = plotting ? Plot2D.getInstance() : null;
    for (RGB.Channel ch : chs) {
//      Plot2D plot = plotting ? Plot2D.getInstance(ch.name()) : null;
      List<Patch> grayScalePatch = lcdTarget.filter.grayScalePatch(ch, true);
      int size = grayScalePatch.size();
      int n = size - 1;
      int index = ch.getArrayIndex();
      ...
      for (int x = 1; x < size; x++) {
        CIEXYZ XYZ = grayScalePatch.get(x).getXYZ();
//        CIEXYZ pureXYZ = CIEXYZ.minus(XYZ, flareXYZ);
//        CIExyY xyY = new CIExyY(pureXYZ);
        CIExyY xyY = new CIExyY(XYZ);
      ...
      double[] relativeLuminanceCurve = DoubleArray.minus(luminanceCurve,
          flareXYZ.Y);
//         pureMaxLuminance[index] =relativeLuminanceCurve[n-1]-
      ...
      double[] YParam = SCurveModel.estimateSCurveParameter(
          relativeLuminanceCurve);
      ...
      PolynomialRegression yRegress = new PolynomialRegression(inputy,
          rgbxyYCurve[index][1], Polynomial.COEF_1.BY_4C);
      xRegress.regress();
      yRegress.regress();
