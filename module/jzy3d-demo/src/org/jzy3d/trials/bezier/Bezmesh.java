...
public class Bezmesh extends AbstractDrawable {
	...
	public void setData(Coord3d[][] points, float xmin, float xmax, int xsteps, int xorder, float ymin, float ymax, int ysteps, int yorder) {
		...
		// prepare a buffer
		controlPointBuffer = BufferUtil.newFloatBuffer(ni * nj * DIM);
		for (int i = 0; i < ni; i++) {
			for (int j = 0; j < nj; j++) {
				for (int k = 0; k < DIM; k++) {
	...
	protected void setupEvaluator(GL gl, float xmin, float xmax, int xsteps, int xorder, float ymin, float ymax, int ysteps, int yorder){
		gl.glMap2f(GL.GL_MAP2_VERTEX_3, xmin, xmax, DIM, xorder, ymin, ymax, nj * DIM, yorder, controlPointBuffer);
		gl.glEnable(GL.GL_MAP2_VERTEX_3);
		//gl.glMapGrid2f(nXSteps, startEvalX, endEvalX, nYSteps, startEvalY, endEvalY);
	}
	
	protected void evaluate(GL gl, Color color, float xmin, float xmax, int xsteps, float ymin, float ymax, int ysteps){
	...
	protected void drawControlPoints(GL gl, Color color, int width){
		gl.glColor4f(color.r, color.g, color.b, color.a);
