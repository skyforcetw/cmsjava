...
public class DefaultGraphDemo
  ...
  public DefaultGraphDemo() {
    ...
    IGraphLayout2d<String>
        layout = StringGraphGenerator.getRandomLayout(graph, GL_LAYOUT_SIZE);
    IGraphFormatter<String, String>
        formatter = new DefaultGraphFormatter<String, String> ();
    formatter.setVertexLabelsDisplayed(false);

    // Create chart
    Quality quality = Quality.Nicest;
    quality.setDepthActivated(false);
    chart = new GraphChart(quality, "swing");
    chart.getView().setAxeBoxDisplayed(false);
    chart.getView().setViewPositionMode(ViewPositionMode.TOP);
    chart.getView().setSquared(false);

    // Build a drawable graph
    final DefaultDrawableGraph2d<String, String> dGraph = new
        DefaultDrawableGraph2d<String, String> ();

    GraphChartMouseController<String, String> mouse = new
        GraphChartMouseController<String, String> (chart);
