...
//				return 10*Math.sin(x/10)*Math.cos(y/20)*x+ x*y;
    ...
    Chart chart = getChart(mapper, xrange, yrange, steps, Quality.Nicest, "awt");
  ...
  public static Chart getChart(Mapper mapper, Range xrange, Range yrange,
                               int steps, Quality quality, String type) {
    // Create the object to represent the function over the given range.
    final Shape surface = (Shape) Builder.buildOrthonormal(new OrthonormalGrid(
        xrange, steps, yrange, steps), mapper);
    ...
    MapperContourMeshGenerator contour = new MapperContourMeshGenerator(mapper,
        xrange, yrange);
    ...
    ContourMesh mesh = contour.getContourMesh(new DefaultContourColoringPolicy(
        myColorMapper), 400, 400, 10, 0, false);
