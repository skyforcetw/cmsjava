...
public class FFT_ implements PlugInFilter {
    ...
    public void fft(ImageProcessor ip, boolean inverse) {
        ...
        if (inverse) {
            ip.resetMinAndMax();
            resultImage = new ImagePlus(imp.getTitle(), ip);
            resultImage.show();
//            new ImagePlus(imp.getTitle(), ip).show();
        } else {
//            ImageProcessor ps = calculateBytePowerSpectrum(fht, maxN);
            ImageProcessor ps = calculateShortPowerSpectrum(fht, maxN);
//            ImageProcessor ps = calculateFloatPowerSpectrum(fht, maxN);
            resultImage = new ImagePlus("FFT", ps);
            resultImage.setProperty("FHT", ip);
//            resultImage.show();
            if (IJ.altKeyDown()) {
    ...
    ImageProcessor calculateBytePowerSpectrum(float[] fht, int maxN) {
        int base;
        float r, scale;
    ...
    ImageProcessor calculateShortPowerSpectrum(float[] fht, int maxN) {
        int base;
        float r, scale;
        float min = Float.MAX_VALUE;
        float max = Float.MIN_VALUE;
        float[] fps = new float[maxN * maxN];
        short[] ps = new short[maxN * maxN];
...
//        scale = (float) (253.0 / (max - min));
        ...
        for (int row = 0; row < maxN; row++) {
            ...
            for (int col = 0; col < maxN; col++) {
                ...
                ps[base + col] = (short) (((r - min) * scale + 0.5) + 1);
            }
        }
        ImageProcessor ip = new ShortProcessor(maxN, maxN, ps, null);
        swapQuadrants(ip);
        return ip;
    }


    ImageProcessor calculateFloatPowerSpectrum(float[] fht, int maxN) {
        int base;
        float r, scale;
        float min = Float.MAX_VALUE;
        float max = Float.MIN_VALUE;
        float[] fps = new float[maxN * maxN];
        int[] ps = new int[maxN * maxN];
...
//        scale = (float) (253.0 / (max - min));
//         scale = (float) (65533.0 / (max - min));
        scale = (float) (4294967293. / (max - min));
        ...
        for (int row = 0; row < maxN; row++) {
            ...
            for (int col = 0; col < maxN; col++) {
                ...
                ps[base + col] = (int) (((r - min) * scale + 0.5) + 1);
            }
        }
        ImageProcessor ip = new FloatProcessor(maxN, maxN, ps);
        swapQuadrants(ip);
        return ip;
    }
    ...
    void amplitude(int row, int maxN, float[] fht, float[] amplitude) {
        ...
        for (int c = 0; c < maxN; c++) {
            l = ((maxN - row) % maxN) * maxN + (maxN - c) % maxN;
            amplitude[base +
                    c] = (float) Math.sqrt(sqr(fht[base + c]) + sqr(fht[l]));
        }
    }
