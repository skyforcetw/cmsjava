...
//                            int a = 1;
    ...
    static boolean isOverlap(int[] position, int index) {
        for (int x = 0; x < index; x++) {
            if (position[x] == position[index] && -1 != position[x] && -1 != position[index]) {
                return true;
            }
        }
        return false;
    ...
    static LinkedList<boolean[][]> five16FramePicker(boolean[][] orgFrame, boolean[][] okPos
    ) {
        ...
        for (int L1 = -1; L1 < okcount[0]; L1++) {
            position[0] = (-1 == L1) ? -1 : okPosIndex[0][L1];

            for (int L2 = -1; L2 < okcount[1]; L2++) {
                position[1] = (-1 == L2) ? -1 : okPosIndex[1][L2];
                if (isOverlap(position, 1)) {
                    continue;
                }

                for (int L3 = -1; L3 < okcount[2]; L3++) {
                    position[2] = (-1 == L3) ? -1 : okPosIndex[2][L3];
                    if (isOverlap(position, 2)) {
                        continue;
                    }

                    for (int L4 = -1; L4 < okcount[3]; L4++) {
                        position[3] = (-1 == L4) ? -1 : okPosIndex[3][L4];
                        if (isOverlap(position, 3)) {
                            continue;
                        }

                        for (int L5 = -1; L5 < okcount[4]; L5++) {
                            position[4] = (-1 == L5) ? -1 : okPosIndex[4][L5];
                            if (isOverlap(position, 4)) {
                                continue;
                            }

                            for (int L6 = -1; L6 < okcount[5]; L6++) {
                                position[5] = (-1 == L6) ? -1 : okPosIndex[5][L6];
                                if (isOverlap(position, 5)) {
                                    continue;
                                }

                                for (int L7 = -1; L7 < okcount[6]; L7++) {
                                    position[6] = (-1 == L7) ? -1 : okPosIndex[6][L7];
                                    if (isOverlap(position, 6)) {
                                        continue;
                                    }

                                    for (int L8 = -1; L8 < okcount[7]; L8++) {
                                        position[7] = (-1 == L8) ? -1 : okPosIndex[7][L8];
                                        if (isOverlap(position, 7)) {
                                            continue;
                                        }
