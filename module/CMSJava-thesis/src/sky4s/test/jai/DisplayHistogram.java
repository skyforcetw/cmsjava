...
public class DisplayHistogram
  ...
  public Dimension getPreferredSize() {
    return new Dimension(width + border.left + border.right,
  ...
  protected void paintComponent(Graphics g) {
    ...
    for (int bin = 0; bin < histogram.getNumBins(0); bin++) {
      ...
      double barStarts = border.top +
          height * (maxCount - counts[bin]) / (1. * maxCount);
    ...
    for (int l = 0; l <= verticalTicks; l++) { // last will be done separately
      ...
      g2d.drawString(label, border.left - 2 - textWidth,
                     border.top + height - l * (height / verticalTicks));
    }
    // Draw the title.
    ...
    g2d.drawString(title, (border.left + width + border.right - textWidth) / 2,
                   28);
  }
