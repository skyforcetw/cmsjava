 ...
 * @version 1.0
 */
public class LCDModelCalibrator implements LCDModelCalibratorConst, Plottable {
    ...
    private RGB getWhiteRGB() {
        if (whiteRGB == null) {
            if (wp.isRGBWhitePoint()) {
                ...
                whiteRGB = model.calculateWhiteRGB(wp.getWhitexyY(),
                        wp.maxWhiteCode,
    ...
    protected RGB[] calibrateByGreen() {
        ...
        // 4.依照G code計算出W的理想亮度曲線
        //==========================================================================
        double[] whiteYArray = codeCalibrator.whiteLuminanceCalibrate(
                caliRGBArray);
        //==========================================================================
        ...
        // 依照目標值, 產生校正結果
        //==========================================================================
        caliRGBArray = codeCalibrator.greenCalibrate(caliRGBArray,
                targetxyYCurve,
    ...
    private RGB[] processGreenArray1(RGB[] caliRGBArray, String filename) {
        ...
        CalibrateUtils.storeRGBArrayExcel(caliRGBArray,
                                          rootDir + "/" + filename,
                                          cp);
        return caliRGBArray;
    }
    ...
    private RGB[] processCalibratedRGBArray2(RGB[] caliRGBArray,
                                             String filename) {
        //==========================================================================
        // 5.將G code量化並避免量化崩潰
        //==========================================================================
        caliRGBArray = CalibrateUtils.quantization(caliRGBArray,
                cp.calibrateBits);
    ...
    protected CIExyY[] productTargetxyYCurveByWhite() {
        ...
        if (ensureWhiteXYZ) {
            ...
            this.setWhiteXYZ(wp.getWhitexyY(),
                             whiteYArray[whiteYArray.length - 1]);
        }
        //==========================================================================
    ...
    protected class CodeCalibrator {
        ...
        protected RGB[] greenCalibrate(RGB[] greenCodeArray,
            ...
            if (multiThread) {
                ...
                for (int x = 0; x < 256; x++) {
                    ...
                    Callable<RGB> callable = new Callable<RGB>() {
                        public RGB call() throws Exception {
                            return rbc.getRB(new CIExyY(XYZ), greenCode.G,
                                             tolerance, false);
                        }
                    };
                ...
                for (int x = 0; x < 256; x++) {
                    ...
                    rgbArray[x] = rbc.getRB(new CIExyY(XYZ), greenCode.G,
                                            tolerance, false);
                }
            }
        ...
        protected RGB[] rgbCalibrate(CIExyY[] targetxyYCurve,
                                     boolean multiThread) {
            if (targetxyYCurve.length != 256) {
                throw new IllegalStateException(
                        "targetxyYCurve.length != 256");
        ...
        protected double[] wGammaCalibrate(RGB whiteRGB) {
            ...
            switch (cp.gamma) {
            case Native:
                double[][] originalGammaCurve = cali.getOriginalWhiteGammaCurve(
                        wp.
                        maxWhiteCode);
        ...
        protected RGB[] greenGammaCalibrate(double maxG) {
            ...
            switch (cp.gamma) {
            case Native:
                double[][] gammaCurve = cali.getOriginalGammaCurve(RGB.Channel.
                        G,
                        maxG);
            ...
            case Scale:
                ...
                break;
            case Custom:
                gArray = cali.gammaCalibrate(RGBBase.Channel.G, cp.customGamma,
                                             maxG);
                break;
