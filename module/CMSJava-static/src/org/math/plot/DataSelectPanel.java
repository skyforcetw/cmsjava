...
public class DataSelectPanel
  ...
  public DataSelectPanel(Object[][] data, int dimension,
    ...
    if (_dimension > parametersNames.length) {
      throw new IllegalArgumentException(
          "Number of parameters must be > to dimension=" + _dimension);
    }
  ...
  void buildRows(int ...selectedaxis) {
    ...
    setPreferredSize(new Dimension(row_width,
                                   row_height * _parametersNames.length));
  ...
  public void setData(Object[][] data) {
    if (data[0].length != _data[0].length) {
      throw new IllegalArgumentException(
          "new data dimension is not consistent with previous one.");
    }
    _data = data;
  ...
  void updateSelectedData() {
    ...
    for (ParameterRow row : rows) {
      boolean isaxis = row.xaxis.isSelected() || row.yaxis.isSelected() ||
          row.zaxis.isSelected();
      if (row._isNumber) {
      ...
      if (!isaxis) {
        if (row._isNumber) {
          row.name.setText(row._paramName + "=[" +
                           row._kernelDoubleValues[row.min.getValue() - 1] +
                           ","
          ...
          row.name.setText(row._paramName + "={" +
                           Array.cat(row.list.getSelectedValues()) + "}");
        }
      }
      else {
        row.name.setText(row._paramName);
    ...
    for (int i = 0; i < _data.length; i++) {
      ...
      for (int j = 0; j < rows.length; j++) {
        ...
        if (!row.xaxis.isSelected() && !row.yaxis.isSelected() &&
            !row.zaxis.isSelected() && !row.check(_data[i][j])) {
          sel = false;
        }
      }
  ...
  public void fireSelectedDataChanged(String from) {
    ...
    switch (_dimension) {
      ...
      case 3:
        System.out.println(Array.cat(new String[] {getSelectedXAxis(),
                                     getSelectedYAxis(), getSelectedZAxis()}));
        break;
    }
    if (sel.length > 0) {
  ...
  class ParameterRow
    ...
    public ParameterRow(String paramName, Object[] values) {
      ...
      if (_isNumber) {
        ...
        max = new JSlider(1, _kernelDoubleValues.length,
    ...
    boolean check(Object value) {
      if (_isNumber) {
        ...
        return (dval >= _kernelDoubleValues[min.getValue() - 1] &&
                dval <= _kernelDoubleValues[max.getValue() - 1]);
      }
      else {
        for (int i = 0; i < list.getSelectedIndices().length; i++) {
          if (_kernelStringValues.get(list.getSelectedIndices()[i]).equals(
              value)) {
            return true;
          }
        }
  ...
  public static void main(String[] args) {
    ...
    DataSelectPanel dsp = new DataSelectPanel(data, 3, "x1", "x2", "x3", "x4",
                                              "x5") {
