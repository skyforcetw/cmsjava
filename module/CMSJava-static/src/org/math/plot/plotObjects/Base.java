...
public class Base {
  ...
  private void setBounds(int i, double Xmi, double Xma) {
    if ( (Xmi <= 0) && (axesScales[i].equalsIgnoreCase(LOGARITHM))) {
      throw new IllegalArgumentException("Error while bounding dimension " +
                                         (i + 1) + " : bounds [" + Xmi + "," +
                                         Xma
                                         +
  ...
  public void roundBounds(int i) {
    ...
    if (axesScales[i].equalsIgnoreCase(LOGARITHM)) {
      setBounds(i, pow(10, floor(log(trueXmin[i]) / log(10))),
                pow(10, ceil(log(trueXmax[i]) / log(10))));
    }
    else if (axesScales[i].equalsIgnoreCase(LINEAR) ||
             axesScales[i].equalsIgnoreCase(STRINGS)) {
      setBounds(i, precisionUnit[i] * (floor(trueXmin[i] / precisionUnit[i])),
                precisionUnit[i] * (ceil(trueXmax[i] / precisionUnit[i])));
    }
