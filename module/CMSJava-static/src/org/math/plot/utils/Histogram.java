...
public class Histogram {
  ...
  public static double[][] histogram_classes(double[] values, double min,
                                             double max, int n) {
  ...
  public static double[][] histogram_classes(double[] values, int n) {
    ...
    return mergeColumns(centers(bounds), fastHistogram(values, bounds));
  }
  ...
  public static double[] histogram(double[] values, double[] bounds) {
    ...
    for (int i = 0; i < values.length; i++) {
      for (int j = 0; j < h.length; j++) {
        if ( ( (bounds[j + 1] - values[i]) * (bounds[j] - values[i]) < 0) ||
            ( (bounds[j] == values[i]))) {
          h[j]++;
        }
      }
  ...
  public static double[] fastHistogram(double[] values, double[] bounds) {
    double[] h = new double[bounds.length - 1];
    for (int i = 0; i < values.length; i++) {
      for (int j = 0; j < h.length; j++) {
        if ( ( (bounds[j + 1] - values[i]) < 0 && (bounds[j] - values[i]) > 0) ||
            ( ( (bounds[j + 1] - values[i]) > 0 && (bounds[j] - values[i]) < 0)) ||
            (bounds[j] == values[i])) {
          h[j]++;
        }
...
//        }
  ...
  public static double[] histogram(double[] values, double min, double max,
                                   int n) {
    double[] bounds = bounds(values, min, max, n);
...
//  }
  ...
  public static double[][] histogram_classes_2D(double[][] values,
                                                double[] boundsX,
                                                double[] boundsY) {
    return insertColumn(centers_2D(boundsX, boundsY),
                        histogram_2D(values, boundsX, boundsY), 2);
  }

  public static double[][] histogram_classes_2D(double[][] values, double minX,
                                                double maxX, int nX,
  ...
  public static double[][] histogram_classes_2D(double[][] values, int nX,
                                                int nY) {
    double[] valuesX = getColumnCopy(values, 0);
    double[] valuesY = getColumnCopy(values, 1);
  ...
  public static double[] histogram_2D(double[][] values, double[] boundsX,
                                      double[] boundsY) {
    ...
    for (int n = 0; n < values.length; n++) {
      for (int i = 0; i < boundsX.length - 1; i++) {
        for (int j = 0; j < boundsY.length - 1; j++) {
          if ( ( ( (boundsX[i + 1] - values[n][0]) * (boundsX[i] - values[n][0]) <
                  0) || ( (boundsX[i] == values[n][0])))
              &&
              ( ( (boundsY[j + 1] - values[n][1]) * (boundsY[j] - values[n][1]) <
                 0) || ( (boundsY[j] == values[n][1])))) {
  ...
  public static double[] histogram_2D(double[][] values, double minX,
                                      double maxX, int nX, double minY,
                                      double maxY, int nY) {
    double[] valuesX = getColumnCopy(values, 0);
    double[] valuesY = getColumnCopy(values, 1);
    double[] boundsX = bounds(valuesX, minX, maxX, nX);
  ...
  public static double[][] histogram_classes_3D(double[][] values, int nX,
                                                int nY, int nZ) {
    double[] valuesX = getColumnCopy(values, 0);
    double[] valuesY = getColumnCopy(values, 1);
    ...
    return insertColumn(centers_3D(boundsX, boundsY, boundsZ),
                        histogram_3D(values, boundsX, boundsY, boundsZ), 3);
  }

  public static double[] histogram_3D(double[][] values, double[] boundsX,
                                      double[] boundsY, double[] boundsZ) {
    double[] h = new double[ (boundsX.length - 1) * (boundsY.length - 1) *
        (boundsZ.length - 1)];
    for (int n = 0; n < values.length; n++) {
      for (int i = 0; i < boundsX.length - 1; i++) {
        for (int j = 0; j < boundsY.length - 1; j++) {
          for (int k = 0; k < boundsZ.length - 1; k++) {
            if ( ( ( (boundsX[i + 1] - values[n][0]) *
                    (boundsX[i] - values[n][0]) < 0) ||
                  ( (boundsX[i] == values[n][0])))
                &&
                ( ( (boundsY[j + 1] - values[n][1]) * (boundsY[j] - values[n][1]) <
                   0) || ( (boundsY[j] == values[n][1]))) &&
                ( ( (boundsZ[k + 1] - values[n][2]) * (boundsZ[k] - values[n][2]) <
                   0) || ( (boundsZ[k] == values[n][2])))) {
  ...
  private static double[][] centers_3D(double[] boundsX, double[] boundsY,
                                       double[] boundsZ) {
    int nb_centers = (boundsX.length - 1) * (boundsY.length - 1) *
        (boundsZ.length - 1);
