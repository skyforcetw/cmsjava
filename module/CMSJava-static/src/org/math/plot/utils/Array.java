...
public class Array {
  ...
  public static HashMap<Double, String> reverseStringMap(HashMap<String, Double>
      map) {
    if (map == null) {
      return null;
    }
  ...
  private static HashMap<String, Double> mapStringArray(double minvalue,
      double step, List<String> array) {
    if (array == null) {
      return null;
    }
  ...
  public static String cat(String columnsSeparator, String rowsSeparator,
                           Object[][] array) {
    String o = "";
    for (int i = 0; i < array.length - 1; i++) {
  ...
  public static double[][] deleteColumns(double[][] x, int ...J) {
    ...
    for (int i = 0; i < array.length; i++) {
      ...
      for (int j = 0; j < J.length - 1; j++) {
        System.arraycopy(x[i], J[j] + 1, array[i], J[j] - j,
                         J[j + 1] - J[j] - 1);
      }
      System.arraycopy(x[i], J[J.length - 1] + 1, array[i],
                       J[J.length - 1] - J.length + 1,
                       x[i].length - J[J.length - 1] - 1);
    }
    return array;
  }
  ...
  public static double[] delete(double[] x, int ...J) {
    ...
    System.arraycopy(x, J[J.length - 1] + 1, array,
                     J[J.length - 1] - J.length + 1,
                     x.length - J[J.length - 1] - 1);
    return array;
  }
  ...
  public static void checkColumnDimension(double[][] M, int n) {
    for (int i = 0; i < M.length; i++) {
      if (M[i].length != n) {
        throwError("row " + i + " have " + M[i].length + " columns instead of " +
                   n + " columns expected.");
      }
    }
  }
