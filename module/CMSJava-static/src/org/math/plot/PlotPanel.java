...
public abstract class PlotPanel
  ...
  public int getAxisCount() {
    return plotCanvas.getGrid().getAxis().length;
  }

  public int[] getAxisPixelCount(int axe) {
    return plotCanvas.getAxisPixelCount(axe);
  }
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 0) {
      ...
      args = new String[] {
          "-3D", "-l", "SOUTH", "-t", "SCATTER", "tmp.dat"};
//      args = new String[] {
//          "-3D", "-l", "SOUTH", "-t", "BAR", "tmp.dat"};
          ...
          if (input_file.exists()) {
            ...
            if (p instanceof Plot2DPanel) {
              ...
              if (type.equals("SCATTER")) {
                n = p2d.addScatterPlot(name,
                                       ASCIIFile.readDoubleArray(input_file));
              }
                ...
                n = p2d
                    .addHistogramPlot(name,
                                      ASCIIFile.readDouble1DArray(input_file),
                                      Integer
                                      .parseInt(type.substring(12,
                    type.length() - 1)));
              }
              else if (type.startsWith("CLOUD2D(")) {
                n = p2d.addCloudPlot(name, ASCIIFile.readDoubleArray(input_file),
                                     Integer.parseInt(type.substring(8,
                    type.indexOf(","))),
                                     Integer.parseInt(type.substring(type.
                    indexOf(",") + 1, type.length() - 1)));
              }
              else {
              ...
              if (type.equals("SCATTER")) {
                ...
                n = p3d.addHistogramPlot(name,
                                         ASCIIFile.readDoubleArray(input_file),
                                         Integer.parseInt(type.substring(12,
                    type.indexOf(","))),
                                         Integer.parseInt(type.substring(type.
                    indexOf(",") + 1, type.length() - 1)));
              }
                ...
                n = p3d.addCloudPlot(name, ASCIIFile.readDoubleArray(input_file),
                                     Integer.parseInt(type.substring(8,
                    type.indexOf(","))),
                                     Integer.parseInt(type.substring(type.
                    indexOf(",") + 1, type.indexOf(",", type.indexOf(",") + 1))),
                                     Integer
                                     .parseInt(type.substring(type.indexOf(",",
                    type.indexOf(",") + 1) + 1, type.length() - 1)));
              }
              else {
  ...
  public double[] getFixedBounds(int axe) {
    return plotCanvas.getFixedBounds(axe);
  }
