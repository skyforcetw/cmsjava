...
public class DensityLayerPlot
    ...
    if (constant_Q == null) {
      for (int i = 0; i < plot.getData().length; i++) {
        ...
        for (int j = 0; j < Q[i].length - 2; j++) {
          ...
          Color c1 = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (norm / (Q[i][j + 1] - Q[i][j]))));
          Color c2 = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (norm / (Q[i][j + 2] - Q[i][j + 1]))));
      ...
      for (int i = 0; i < plot.getData().length; i++) {
        ...
        for (int j = 0; j < constant_Q.length - 2; j++) {
          ...
          Color c1 = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 *
                                      (norm / (constant_Q[j + 1] - constant_Q[j]))));
          Color c2 = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 *
                                      (norm /
                                       (constant_Q[j + 2] - constant_Q[j + 1]))));
          draw.setGradient(d1, c1, d2, c2);
          draw.drawLine(d1, d2);
        }
      }
  ...
  public static void main(String[] args) {
    ...
    p2.getPlot(0).addQuantiles(1, new double[] { /*-3,-2,*/ -4, -2, -0.5, 0,
                               0.5, 2, 4 /*,2,3*/});
