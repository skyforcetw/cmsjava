...
public class GridPlot3D
    ...
    if (draw_lines) {
      ...
      for (int i = 0; i < X.length; i++) {
        for (int j = 0; j < Y.length - 1; j++) {
          draw.drawLine(new double[] {X[i], Y[j], Z[j][i]}, new double[] {X[i],
                        Y[j + 1], Z[j + 1][i]});
        }
      }
      ...
      for (int j = 0; j < Y.length; j++) {
        for (int i = 0; i < X.length - 1; i++) {
          draw.drawLine(new double[] {X[i], Y[j], Z[j][i]},
  ...
  public static void main(String[] args) {
    ...
    int num = p.addGridPlot("toto", X, Y, Z);
//    int num = p.addGridPlot("", Z);
    ( (GridPlot3D) p.getPlot(num)).fill_shape = false;
