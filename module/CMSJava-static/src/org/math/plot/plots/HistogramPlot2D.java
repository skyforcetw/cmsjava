...
public class HistogramPlot2D
  ...
  public HistogramPlot2D(String n, Color c, double[][] _XY, double w,
                         double _offsetCenter_perWidth, double _factorWidth) {
    super(n, c);
    XY = _XY;
    width_constant = w;
  ...
  private void build() {
    if (width_constant > 0) {
      ...
      for (int i = 0; i < XY.length; i++) {
        topLeft[i] = new double[] {
            XY[i][0] - factorWidth * width_constant / 2 +
            (offsetCenter_perWidth - 0.5) * width_constant, XY[i][1]};
        topRight[i] = new double[] {
            XY[i][0] + factorWidth * width_constant / 2 +
            (offsetCenter_perWidth - 0.5) * width_constant, XY[i][1]};
        bottomLeft[i] = new double[] {
            XY[i][0] - factorWidth * width_constant / 2 +
            (offsetCenter_perWidth - 0.5) * width_constant, 0};
        bottomRight[i] = new double[] {
            XY[i][0] + factorWidth * width_constant / 2 +
            (offsetCenter_perWidth - 0.5) * width_constant, 0};
      }
    }
      ...
      for (int i = 0; i < XY.length; i++) {
        ...
        bottomLeft[i] = new double[] {
            XY[i][0] - factorWidth * widths[i] / 2 +
            (offsetCenter_perWidth - 0.5) * widths[i], 0};
        bottomRight[i] = new double[] {
            XY[i][0] + factorWidth * widths[i] / 2 +
            (offsetCenter_perWidth - 0.5) * widths[i], 0};
      }
  ...
  public void plot(AbstractDrawer draw, Color c) {
    ...
    for (int i = 0; i < XY.length; i++) {
      ...
      if (fill_shape) {
        draw.fillPolygon(0.2f, bottomLeft[i], topLeft[i], topRight[i],
                         bottomRight[i]);
      }
    }
  }
