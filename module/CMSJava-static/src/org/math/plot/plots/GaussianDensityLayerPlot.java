...
public class GaussianDensityLayerPlot
  ...
  public GaussianDensityLayerPlot(Plot p, int ax, double sigma) {
    ...
    for (int i = 0; i < gausspdf_sigma.length; i++) {
      for (int j = 0; j < 4; j++) {
        gausspdf_sigma[i][j] = (float) (
            /*1.0 / Math.sqrt(2 * Math.PI * constant_sigma * constant_sigma) */
            Math.exp( - (j * j)
  ...
  public GaussianDensityLayerPlot(Plot p, int ax, double[] sigma) {
    ...
    if (sigma != null) {
      ...
      for (int i = 0; i < gausspdf_sigma.length; i++) {
        for (int j = 0; j < 4; j++) {
          gausspdf_sigma[i][j] = (float) (
              /*1.0 / Math.sqrt(2 * Math.PI * sigma[i] * sigma[i]) */Math.exp( - (
                  j * j) / (2.0 * sigma[i] * sigma[i])));
        }
      }
    }
  ...
  public void plot(AbstractDrawer draw, Color c) {
    ...
    if (constant_sigma == 0) {
      for (int i = 0; i < plot.getData().length; i++) {
        gradC_0sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                                 (int) (255.0 * (gausspdf_sigma[i][0])));
        gradC_1sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                                 (int) (255.0 * (gausspdf_sigma[i][1])));
        gradC_2sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                                 (int) (255.0 * (gausspdf_sigma[i][2])));
        gradC_3sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                                 (int) (255.0 * (gausspdf_sigma[i][3])));
      ...
      gradC_0sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (gausspdf_sigma[0][0])));
      gradC_1sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (gausspdf_sigma[0][1])));
      gradC_2sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (gausspdf_sigma[0][2])));
      gradC_3sigma = new Color(c.getRed(), c.getGreen(), c.getBlue(),
                               (int) (255.0 * (gausspdf_sigma[0][3])));
