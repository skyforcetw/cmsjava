...
public abstract class Projection {
  ...
  public void dilate(int[] screenOrigin, double[] screenRatio) {
    ...
    for (int i = 0; i < draw.canvas.base.dimension + 1; i++) {
      ...
      baseScreenCoords[i][0] = (int) ( (baseScreenCoords[i][0] - screenOrigin[0]) /
                                      screenRatio[0]);
      baseScreenCoords[i][1] = (int) ( (baseScreenCoords[i][1] - screenOrigin[1]) /
                                      screenRatio[1]);
      // System.out.println(" -> baseScreenCoords["+i+"] =
      // "+baseScreenCoords[i][0]+" , "+baseScreenCoords[i][1]);
    }
...
//  public double[] screenUnProjection(int ...sC) {
//    double[] sCd = DoubleArray.toDoubleArray(sC);
//    if (draw.screen_offset != null) {
//      sCd[0] -= draw.screen_offset[0];
//      sCd[1] -= draw.screen_offset[1];
//    }
//
//    if (draw.base_offset != null) {
//      for (int i = 0; i < draw.canvas.base.dimension; i++) {
//        sCd[0] -= draw.base_offset[i] *
//            (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0]);
//        sCd[1] -= draw.base_offset[i] *
//            (baseScreenCoords[i + 1][1] - baseScreenCoords[0][1]);
//      }
//    }
//
//    double[] origin = new double[2];
//    origin[0] = baseScreenCoords[0][0];
//    origin[1] = baseScreenCoords[0][1];
//    double[] diff = DoubleArray.minus(sCd, origin);
...
//        double logpC = (diff[0] /
//                        (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0])) *
//            (log(draw.canvas.base.baseCoords[i + 1][i]) -
//             log(draw.canvas.base.baseCoords[0][i])) +
//            log(draw.canvas.base.baseCoords[0][i]);
//        pC[i] = Math.pow(10, logpC);
//
////        sC[0] +=
////            ( (log(pC[i]) - log(draw.canvas.base.baseCoords[0][i])) /
////             (log(draw.canvas.base.baseCoords[i + 1][i]) -
////              log(draw.canvas.base.baseCoords[0][i])))
////            * (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0]);
////        sC[1] +=
...
//               draw.canvas.base.axesScales[i].equalsIgnoreCase(Base.STRINGS)) {
//        double v0 = (diff[0] /
//                     (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0])) *
//            (draw.canvas.base.baseCoords[i + 1][i] -
//             draw.canvas.base.baseCoords[0][i]) +
//            draw.canvas.base.baseCoords[0][i];
//        double v1 = (diff[1] /
//                     (baseScreenCoords[i + 1][1] - baseScreenCoords[0][1])) *
//            (draw.canvas.base.baseCoords[i + 1][i] -
...
//        pC[i] = v[i];
////        sC[0] +=
////            ( (pC[i] - draw.canvas.base.baseCoords[0][i]) /
////             (draw.canvas.base.baseCoords[i + 1][i] -
////              draw.canvas.base.baseCoords[0][i]))
////            * (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0]);
////        sC[1] +=
...
//    dimension = 2;
    ...
    if (draw.base_offset != null) {
      for (int i = 0; i < draw.canvas.base.dimension; i++) {
        sC[0] += draw.base_offset[i] *
            (baseScreenCoords[i + 1][0] - baseScreenCoords[0][0]);
        sC[1] += draw.base_offset[i] *
            (baseScreenCoords[i + 1][1] - baseScreenCoords[0][1]);
      }
    }
    ...
    int[] result = new int[] {
        (int) sC[0], (int) sC[1]};
//    double[] un = screenUnProjection(result);
    return result;
  }
...
//    System.out.println(Double.doubleToRawLongBits(d));
    System.out.println(Double.longBitsToDouble( (int) Double.doubleToLongBits(d)));
    System.out.println(Double.longBitsToDouble( (int) Double.doubleToLongBits(d)));
