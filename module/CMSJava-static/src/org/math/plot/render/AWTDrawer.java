...
public abstract class AWTDrawer
  ...
  /**
   * 座標軸的最小值和最大值對應的pixel位置
   * @return int[][]
   */
  public int[][] getBoundsSc() {
    double[][] pC = getBounds();

    int[] sC0 = this.project(pC[0]);
    int[] sC1 = this.project(pC[1]);
    int[][] sC = new int[][] {
        sC0, sC1};
    return sC;
  }
  ...
  protected double[][] getBounds() {
    ...
    for (int x = 0; x < 2; x++) {
      double[] bounds = this.canvas.getFixedBounds(x);
      pC[0][x] = bounds[0];
      pC[1][x] = bounds[1];
    }
    return pC;
  ...
  public double[] getUnits() {
    ...
    int[] sC1 = this.project(pC[1]);
    int[] sC = new int[] {
        Math.abs(sC0[0] - sC1[0]), Math.abs(sC0[1] - sC1[1])};
    double[] unit = new double[2];
    unit[0] = (pC[1][0] - pC[0][0]) / sC[0];
    unit[1] = (pC[1][1] - pC[0][1]) / sC[1];
    return unit;
  }
  ...
  public void center(int ...t) {
    ...
    for (int x = 0; x < dimension; x++) {
      ...
      this.canvas.setFixedBounds(x, result[0], result[1]);
    }
  ...
  private void drawLine(int[] ...c) {
    ...
    switch (line_type) {
      case CONTINOUS_LINE:
        s = new BasicStroke(line_width, BasicStroke.CAP_BUTT,
    ...
    switch (dot_type) {
      case ROUND_DOT:
        switch (dot_fill) {
          default:
          case DOT_FILL_WHOLE:
            comp2D.fillOval(sC[0] - dot_radius, sC[1] - dot_radius,
                            2 * dot_radius,
                            2 * dot_radius);

            break;
          case DOT_FILL_INSIDE:
            comp2D.fillOval(sC[0] - dot_radius, sC[1] - dot_radius,
                            2 * dot_radius,
                            2 * dot_radius);
            setColorBlack(true);
            comp2D.drawOval(sC[0] - dot_radius, sC[1] - dot_radius,
                            2 * dot_radius,
                            2 * dot_radius);
            setColorBlack(false);
            break;
      ...
      case CROSS_DOT:
        ...
        break;
      case CROSS2_DOT:
        comp2D.drawLine(sC[0] - dot_radius, sC[1],
                        sC[0] + dot_radius, sC[1]);
        comp2D.drawLine(sC[0], sC[1] - dot_radius,
                        sC[0], sC[1] + dot_radius);
        break;
      case SQUARE_DOT:
        switch (dot_fill) {
          default:
          case DOT_FILL_WHOLE:
            comp2D.fillRect(sC[0] - dot_radius, sC[1] - dot_radius,
                            2 * dot_radius,
                            2 * dot_radius);
            break;
          case DOT_FILL_INSIDE:
            comp2D.fillRect(sC[0] - dot_radius, sC[1] - dot_radius,
                            ...
                            2 * dot_radius);
            setColorBlack(true);
            comp2D.drawRect(sC[0] - dot_radius, sC[1] - dot_radius,
                            2 * dot_radius,
                            2 * dot_radius);
            setColorBlack(false);
            break;
  ...
  /**
   * 由於傳進來的是 x,y,width,height
   * 需要轉成pixel尺度才能繪圖.
   *
   * 其中width的部分要用pixel尺度推算才是正確的值
   *
   *
   * @param pC double[][]
   */
  public void drawOval(double[] ...pC) {
    int[][] sC = new int[pC.length][];
    int[][] sC2 = new int[pC.length][];
    for (int i = 0; i < sC.length; i++) {
      sC[i] = projection.screenProjection(pC[i]);
      double[] pC4sC2 = {
          pC[i][0] + pC[i][2], pC[i][1] + pC[i][3]};
      sC2[i] = projection.screenProjection(pC4sC2);

      sC2[i][0] = (sC2[i][0] - sC[i][0]);
      sC2[i][1] = (sC[i][1] - sC2[i][1]);

      sC[i][0] = sC[i][0] - (int) Math.round(sC2[i][0] / 2.);
      sC[i][1] = sC[i][1] - (int) Math.round(sC2[i][1] / 2.);

      comp2D.drawOval(sC[i][0], sC[i][1], sC2[i][0], sC2[i][1]);
    }
  }
  ...
  public void fillPolygon(float alpha, double[] ...pC) {
    ...
    comp2D.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
        alpha));
    comp2D.fillPolygon(x, y, c.length);
    comp2D.setComposite(cs);
  }

  public void drawImage(Image img, float alpha, double[] _xyzSW,
                        double[] _xyzSE, double[] _xyzNW) {
    Composite cs = comp2D.getComposite();
    comp2D.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
        alpha));
    AffineTransform t = getAffineTransform(img.getWidth(canvas),
                                           img.getHeight(canvas), _xyzSW,
                                           _xyzSE, _xyzNW);
    if (t != null) {
  ...
  public AffineTransform getAffineTransform(int width, int height,
                                            double[] _xyzSW, double[] _xyzSE,
                                            double[] _xyzNW) {
    ...
    double[] vectWE = {
        (double) cornerSE[0] - (double) cornerSW[0],
        (double) cornerSE[1] - (double) cornerSW[1]};
    double normvectWE = sqrt(sqr(vectWE[0]) + sqr(vectWE[1]));
    double[] vectSN = {
        (double) cornerNW[0] - (double) cornerSW[0],
        (double) cornerNW[1] - (double) cornerSW[1]};
    double normvectSN = sqrt(sqr(vectSN[0]) + sqr(vectSN[1]));
    double angleSW = acos( (vectWE[0] * vectSN[0] + vectWE[1] * vectSN[1]) /
                          (normvectWE * normvectSN));
    ...
    t.scale(normvectWE * cos(angleSW - PI / 2) / (double) width,
            normvectSN / (double) height);
    ...
    if (isDiff(_cornerSE_tr, cornerSE)) {
      double[] vectSE_NW_1 = {
          (double) cornerNW[0] - (double) cornerSE[0],
          (double) cornerNW[1] - (double) cornerSE[1]};
      double[] vectSE_NW_2 = {
      ...
      double cos_angle = ( ( (vectSE_NW_1[0] * vectSE_NW_2[0] +
                              vectSE_NW_1[1] * vectSE_NW_2[1]) /
                            (normvect_1 * normvect_2)));
      double vect = (vectSE_NW_1[0] * vectSE_NW_2[1] -
                     vectSE_NW_1[1] * vectSE_NW_2[0]);
  ...
  private void setColorBlack(boolean black) {
    if (black) {
      originalColor = comp2D.getColor();
      comp2D.setColor(Color.black);
    }
    else {
      comp2D.setColor(originalColor);
    }
  }

  private void setColorHalf(boolean half) {
    if (half) {
      originalColor = comp2D.getColor();
      Color c = new Color(originalColor.getRed() * 2 / 3,
                          originalColor.getGreen() * 2 / 3,
                          originalColor.getBlue() * 2 / 3);
      comp2D.setColor(c);
    }
    else {
      comp2D.setColor(originalColor);
