...
public class Plot3DCanvas
  ...
  public Plot3DCanvas(double[] min, double[] max, String[] axesScales,
                      String[] axesLabels) {
  ...
  public void initBasenGrid(double[] min, double[] max) {
    initBasenGrid(min, max, new String[] {LINEAR, LINEAR, LINEAR},
                  new String[] {"X", "Y", "Z"});
  }
  ...
  public int addBoxPlot(String name, Color c, double[][] XYdX) {
    return addPlot(new BoxPlot3D(getColumnsRangeCopy(XYdX, 0, 2),
                                 getColumnsRangeCopy(XYdX, 3, 5), c, name));
  }
  ...
  public int addHistogramPlot(String name, Color c, double[][] XYdX) {
    return addPlot(new HistogramPlot3D(name, c, getColumnsRangeCopy(XYdX, 0, 2),
                                       getColumnsRangeCopy(XYdX, 3, 4)));
  }

  public int addHistogramPlot(String name, Color c, double[][] XY, int nX,
                              int nY) {
    double[][] XYZ = histogram_classes_2D(XY, nX, nY);
    return addPlot(new HistogramPlot3D(name, c, XYZ, XYZ[1][0] - XYZ[0][0],
                                       XYZ[nX][1] - XYZ[0][1]));
  }
  ...
  public int addHistogramPlot(String name, Color c, double[][] XY,
    ...
    return addPlot(new HistogramPlot3D(name, c, XYZ, XYZ[1][0] - XYZ[0][0],
                                       XYZ[boundsX.length - 1][1] - XYZ[0][1]));
  }
  ...
  public int addGridPlot(String name, Color c, double[][] XYZMatrix) {
    ...
    double[][] Z = getSubMatrixRangeCopy(XYZMatrix, 1, XYZMatrix.length - 1, 1,
                                         XYZMatrix[0].length - 1);

    return addGridPlot(name, c, X, Y, Z);
  }

  public int addCloudPlot(String name, Color c, double[][] sampleXYZ, int nX,
                          int nY, int nZ) {
    double[][] XYZh = histogram_classes_3D(sampleXYZ, nX, nY, nZ);
    return addPlot(new CloudPlot3D(name, c, XYZh, XYZh[1][0] - XYZh[0][0],
                                   XYZh[nX][1] - XYZh[0][1],
                                   XYZh[nX][2] - XYZh[0][2]));
  }
...
//                       abs(mouseCurent[0] - mouseClick[0]), abs(mouseCurent[1]
//            - mouseClick[1]));
        drawZoomRect(e);
        break;
      case ROTATION:
        int[] t = new int[] {
            mouseCurent[0] - mouseClick[0], mouseCurent[1] - mouseClick[1]};
...
//        System.out.println(Arrays.toString(t));
  ...
  public void setView(double _theta, double _phi) {
    ( (AWTDrawer3D) draw).setView(_theta, _phi);
    repaint();
  }
}
