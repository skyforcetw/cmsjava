...
public class Slicing {
  ...
  public Slicing(double[][] m, int[] n, String bounds) {
    ...
    if (bounds.equals(UNIFORM_BOUNDS)) {
      ...
      throw new IllegalArgumentException("The bounds type : " + bounds +
                                         "is unknown. You must specify AUTO or UNIFORM.");
    }
  ...
  public double[][] getSlicingMatrix() {
    ...
    for (int i = 0; i < slice.length; i++) {
      ...
      System.arraycopy(slice[i].width, 0, X[i], numDimensions + 1,
                       numDimensions);
    }
    return X;
  }
  ...
  private void setSlicesBounds(double[][] bounds) {
    ...
    for (int i = 0; i < slice.length; i++) {
      for (int j = 0; j < numDimensions; j++) {
        ...
        sliceCenter[j] = (bounds[j][counter[j] + 1] + bounds[j][counter[j]]) /
            2;
      }
  ...
  private void countFromBounds() {
    ...
    for (int i = 0; i < numOE; i++) {
      ...
      if (inv.size() == 0) {
        ...
        for (int j = 0; j < slice.length; j++) {
          S +=
              slice[ /* ( (Integer) inv.get(j)).intValue() */j].toString(
              "slice[" +
              j + "]");
        }
        ...
        throw new IllegalArgumentException("The element = " +
            ...
            getRowsCopy(M, i, i)) + " is in 0 slices :" + S);
  ...
  private class Slice {
    ...
    public boolean isIn(double[] x) {
      ...
      for (int j = 0; j < x.length; j++) {
        ...
        boolean inj = ( (Math.abs(x[j] - center[j]) - width[j] / 2) <=
                       0.0
                       /*epsilon*/);
    ...
    public String toString(String s) {
      ...
      st.append("  min = " +
                DoubleArray.toString(minus(center, times(width, 0.5))) +
                "  max = " + DoubleArray.toString(plus(center, times(width, 0.5)))
                + "\n");
