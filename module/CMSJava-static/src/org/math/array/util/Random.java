...
public class Random {
  ...
  public static double normal(double mu, double sigma) {
    double x = mu +
        sigma * Math.cos(2 * Math.PI * raw()) * Math.sqrt( -2 * Math.log(raw()));
    return x;
  }
  ...
  public static double triangular(double min, double max) {
    double x = min / 2 + (max - min) * raw() / 2 + min / 2 +
        (max - min) * raw() / 2;
    return x;
  }
   ...
   * @return A double.
   */
  public static double rejection(Function fun, double maxFun, double min,
                                 double max) {
    double try_x;
    double try_y;
    do {
