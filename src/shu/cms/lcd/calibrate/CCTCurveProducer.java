...
public class CCTCurveProducer {
  ...
  protected CCTCurveProducer(LCDModel model, double[] idealWhiteYArray,
    ...
    if (idealWhiteYArray != null) {
      if (idealWhiteYArray.length != 256) {
        throw new IllegalArgumentException("idealWhiteYArray.length != 256");
      }
  ...
  protected final class CorrectedxyYCurve {
     ...
     */
    protected final PolynomialRegression simulateRegression(double dabThreshold) {
      if (p == null) {
      ...
      Polynomial.COEF_1 coef = PolynomialRegression.
          findBestPolynomialCoefficient1(doubleCodeArray, CCTArray);
      PolynomialRegression regress = new PolynomialRegression(doubleCodeArray,
          CCTArray, coef);
      regress.regress();
...
//        xyYCurve[x] = xyY;
      ...
      //========================================================================
      CIExyY[] dimxyYCurve = getPredictedxyYCurve(xyYCurve[1], xyYCurve[size], false);
...
//      int size = 2;
      ...
      CompositePolynomialRegression regress = new
          CompositePolynomialRegression(
              new double[] {adaptiveCode, p.startCode}, output[0], output[1],
   ...
   * @version 1.0
   */
  protected class AdaptivexyYCurve
      extends AdaptivexyYCurveBase {
    private double dab;
...
//      this.end = adaptiveEnd;
  ...
  public final static CIExyY CCT2xyY(int tempK, CCTStyle cctStyle) {
    switch (cctStyle) {
      case DIlluminant:
        ...
        case Blackbody:
          CIEXYZ XYZ = CorrelatedColorTemperature.getSpectraOfBlackbodyRadiator(
  ...
  public final static class CCTParameter {
    ...
    public CCTParameter(int startCode, int codeInterval, int whiteCCT,
                        CIEXYZ black, CCTStyle cctStyle, ColorProofParameter p) {
      this.startCode = startCode;
      this.codeInterval = codeInterval;
  ...
  public static void main(String[] args) throws Exception {
    ...
    ViewingConditions vc = new ViewingConditions(Illuminant.getD65WhitePoint(),
                                                 0.1, 20,
                                                 Surround.Dark,
                                                 "Dark");
    ...
    for (int x = 1; x <= 50; x++) {
      ...
      // ipt
      //========================================================================
      double iptp = Interpolation.linear(1, 50, code1ipt.P, code50ipt.P, x);
      double iptt = Interpolation.linear(1, 50, code1ipt.T, code50ipt.T, x);
      IPT ipt = IPT.fromXYZ(XYZ, white);
      ipt.P = iptp;
      ipt.T = iptt;
      ...
      System.out.print(Math.abs(iptuvp[0] - up) + " " + Math.abs(iptuvp[1] - vp));
   ...
   * @version 1.0
   */
  protected final class UniformAdaptivexyYCurve
      extends AdaptivexyYCurveBase {
    //code與均勻色彩空間中明度的關係
    ...
    protected UniformAdaptivexyYCurve(int adaptiveStart, ColorSpace colorSpace) {
      this.start = adaptiveStart;
  ...
  protected final CIExyY[] getRegressionxyYCurve(int startcode,
    ...
    Polynomial.COEF_1[] coefs = CompositePolynomialRegression.
        findBestPolynomialCoefficient1(codeArray, coordAArray, coordBArray,
                                       CompositePolynomialRegression.Type.
                                       Series);

    CompositePolynomialRegression regress = new CompositePolynomialRegression(
    ...
    double[][] actual = new double[][] {
        code1xyY.getuvPrimeValues(), startcodexyY.getuvPrimeValues()};
...
//    }
  ...
  protected final static Operator getOperator(CompositePolynomialRegression
    ...
    Operator op = Operator.getAdjustOperator(actualOutput[0], actualOutput[1],
                                             predict[0], predict[1]);
    return op;
  }
}
