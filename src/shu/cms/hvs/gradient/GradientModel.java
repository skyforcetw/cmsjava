...
public abstract class GradientModel {
   ...
   * @param height int
   */
  protected void setupImage(int start, int end, boolean R, boolean G, boolean B,
                            int scale, int width, int height) {
    image = GradientShowerFrame.getImage(new Dimension(width, height), start,
                                         end, R, G, B, false, false, false,
                                         scale);
    this.reset();
  }
  ...
  private double[][][] getXYZImage() {
    if (image != null) {
      DeviceIndependentImage di = DeviceIndependentImage.getInstance(image,
          pcs);
