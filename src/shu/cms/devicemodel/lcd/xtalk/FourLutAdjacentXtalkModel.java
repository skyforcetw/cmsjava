 ...
 * @version 1.0
 */
public class FourLutAdjacentXtalkModel
   ...
   */
  public FourLutAdjacentXtalkModel(LCDTarget lcdTarget,
                                   LCDTarget xtalkLCDTarget,
                                   RGBBase.Channel channelVI,
                                   XTalkProperty xtalkProperty,
                                   XTalkProperty chVIIProperty) {
  ...
  private XTalkProperty chVIIProperty = XTalkProperty.getOppositeProperty(this.
      property);
  ...
  protected Factor[] _produceFactor() {
    ...
    double[][] bCorrectLut = getCorrectLUT(RGBBase.Channel.B, codeValues,
                                           codeValues);
    double[][][] correctLuts = new double[][][] {
        rCorrectLut, gCorrectLut, bCorrectLut};

    //==========================================================================
    // Reconstructor
    //==========================================================================
    fourthReconstructor = new FourthLutXTalkReconstructor(mmModel, property);
    double[][][] fourthCorrectLut = getCorrectLUT(channelVI, codeValues,
                                                  chVIIProperty,
                                                  fourthReconstructor);

    correctLuts[chVIIProperty.getAdjacentChannel(channelVI).getArrayIndex()] =
        fourthCorrectLut[0];
    //==========================================================================
    ...
    // Eliminator
    //==========================================================================
    eliminator = new XTalkEliminator(codeValues, codeValues, correctLuts[0],
                                     correctLuts[1], correctLuts[2]);
    chVIEliminator = new XTalkEliminator(codeValues, codeValues,
                                         fourthCorrectLut[1], false);
    //==========================================================================
    return new Factor[3];
  }
...
//    int adfSize = adjacentChannelValues.length;
    ...
    for (int x = 0; x < size; x++) {
      for (int y = 0; y < size; y++) {
        ...
        double selfCorrectValue = xtalkRGB.getValue(selfChannel) -
            keyRGB.getValue(selfChannel);
        double adjCorrectValue = xtalkRGB.getValue(adjChannel) -
            keyRGB.getValue(adjChannel);
  ...
  protected final RGB correctRGB(final RGB rgb, boolean uncorrect) {
    ...
    if (channelVICorrect) {
      ...
      double adjValues = rgb.getValue(chVIIProperty.getAdjacentChannel(
          channelVI));
  ...
  public Plot3D[] plotCorrectLUT() {
    ...
    for (int x = 0; x < 3; x++) {
      ...
      plots[x] = plot3DLUT(ch, correctLut[x], ch.color, property,
                           " Correct LUT", "Correct code");
    }

    double[][][] correctLut2 = chVIEliminator.correctLut;
    plots[3] = plot3DLUT(channelVI, correctLut2[0], Color.black,
                         chVIIProperty, " Correct LUT", "Correct code");
