...
public class LutBtoATypeTest extends TestCase
{
	private byte[] ba = new byte[]
	{
		(byte)0x6D,(byte)0x42,(byte)0x41,(byte)0x20,	//signature
	...
	public void testConstructors()
	{
		try
		{
			LutBtoAType empty = new LutBtoAType();
			assertEquals(empty.SIGNATURE, empty.getSignature().intValue());

			LutBtoAType inst1 = new LutBtoAType(ba);
	...
	{
		LutBtoAType data = new LutBtoAType();
		try
		{
			data.fromByteArray(ba, 0, 0);
			//byte[] res = data.toByteArray();
			assertEquals(LutBtoAType.SIGNATURE, data.getSignature().intValue());
	...
	public void testToByteArray()
	{
		try
		{
			LutBtoAType ct = new LutBtoAType(ba);
		...
		try
		{
			LutBtoAType ct = new LutBtoAType();
			byte[] bac = ct.toByteArray();
