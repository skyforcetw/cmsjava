...
public class Response16Number implements Streamable
{
	...
	private	UInt16Number		deviceCode_;	//device code in range(0..FFFF)
	private	short				reserved_;		//must be zero
	private	S15Fixed16Number	measurement_;	//measurement value
	...
	public byte[] toByteArray() throws ICCProfileException
	{
		...
		if (all.length != this.SIZE)
			throw new ICCProfileException("wrong size", ICCProfileException.WrongSizeException);
	...
	public String toXmlString(String name)
	{
		...
		if (name==null || name.length()<1)
			...
			sb.append("<response16Number name=\""+name+"\">");
