...
{
	private	Signature	deviceMakerSignature_;	//0, from corresponding profile's header
	private	Signature	deviceModelSignature_;
	private	DeviceAttribute	deviceAttributes_;
	private	UInt32Number	deviceTechInfo_;	//profile's technology signature or 0
	...
	public void fromByteArray(byte[] byteArray, int offset, int len) throws ICCProfileException
	{
		...
		int idx = offset + 20;
		if (idx+16 > byteArray.length)
			this.deviceMfgDescTag_ = new MultiLocalizedUnicodeType();
		else
			this.deviceMfgDescTag_ = new MultiLocalizedUnicodeType(byteArray, idx);
		idx += deviceMfgDescTag_.size();
		if (idx+16 > byteArray.length)
			this.deviceModelDescTag_ = new MultiLocalizedUnicodeType();
		else
			this.deviceModelDescTag_ = new MultiLocalizedUnicodeType(byteArray, idx);
	}

	public byte[] toByteArray() throws ICCProfileException
	{
		if (this.deviceMakerSignature_==null)
	...
	public String toXmlString(String name)
	{
		...
		if (name==null || name.length()<1)
			sb.append("<profileDesc sig=\"\">");
		else
			sb.append("<profileDesc name=\""+name+"\" sig=\"\">");
