...
/**
 * ICCProfileWriter implements the ICCProfileOutput interface for writing ICC profile data to a disk file.
 *
 * @author Ted Wen
 * @version 0.1
 * @update 2006-10-27
 ...
 *
 * To save a ICCProfile object, create an ICCProfileWriter object and pass to the ICCProfile.save
 * method as the argument.  The close() method must be called after saving the profile data.
 */
public class ICCProfileWriter implements ICCProfileOutput
{
	private static Logger logger = Logger.getLogger(ICCProfileWriter.class.getName());
	 ...
	 */
	public ICCProfileWriter(String filename) throws ICCProfileException
	{
		logger.finest("Enter ICCProfileWriter('"+filename+"')");
		try
		{
		...
		logger.finest("Leave ICCProfileWriter");
	}
	...
	{
		logger.finest("Enter ICCProfileWriter.write(byte[])");
		...
		logger.finest("Leave ICCProfileWriter.write(byte[])");
	}
	...
	{
		logger.finest("Enter ICCProfileWriter.write(byte[],index,len)");
		if (this.fileHandle_ == null)
		{
			logger.warning("Profile not created");
			throw new ICCProfileException("Profile file not created");
		...
		{
			logger.severe("ICCProfileWriter.write() index outside range");
			throw new ICCProfileException("Write index outside of data range");
		}
		if (index + len > data.length)
		{
			logger.severe("ICCProfileWriter.write() len+index outside range");
		...
		logger.finest("Leave ICCProfileWriter.write(byte[],index,len)");
	}
...
/*	public static void saveProfile(ICCProfile profile, String filename) throws ICCProfileException
	{
		logger.finest("Enter ICCProfileWriter.saveProfile");

		ICCProfileWriter writer = new ICCProfileWriter(filename);
		writer.writeProfile(profile);

		logger.finest("Leave ICCProfileWriter.saveProfile");
	}
*/
	/**
...
/*	public void writeProfile(ICCProfile profile) throws ICCProfileException
	{
		...
		if (profile == null)
		{
			logger.severe("null param with ICCProfileWriter.writeProfile()");
			return;
		}
