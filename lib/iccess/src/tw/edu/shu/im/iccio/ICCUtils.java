...
public class ICCUtils
{
	...
	{
		if (bar==null || src==null)
			throw new ICCProfileException("ICCUtils.appendByteArray():null argument", ICCProfileException.IllegalArgumentException);

		byte[] ba = src.toByteArray();
		if (bar.length < offset + ba.length)
			throw new ICCProfileException("ICCUtils.appendByteArray():byte array not big enough", ICCProfileException.WrongSizeException);
	 ...
	 * For example, convert(data, "EUC_CN", "UTF-16BE");
	 *	convert(data, "UTF-16BE", "Big5");
	 * @param data - byte array containing bytes in source encoding.
	 * @param srcEncoding - source encoding
	 * @param targetEncoding - target encoding
	 * @return - byte array of converted codes
	 */
/*	public static byte[] convert(byte[] data, String srcEncoding, String targetEncoding) throws ICCProfileException
	{
		try
		{
	      ...
	      return str.getBytes(targetEncoding);
		}
		catch (java.io.UnsupportedEncodingException e)
		{
			throw new ICCProfileException("convert: "+e.getMessage(),ICCProfileException.UnsupportedEncodingException);
		}
	}*/
