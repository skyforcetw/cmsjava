...
{
	private	Signature	measureUnitSignature_;	//0
	private	UInt32Number[]	channelMeasures_;	//4..m number of measurements for each channel, determined by numChannels in ResponseCurveSet16Type.
	private	XYZNumber[]	patchMeasures_;		//num-of-channels measurements of patch
	private	Response16Number[]	responseArray_;	//num-of-channels response arrays
	...
	public void fromByteArray(byte[] byteArray, int offset, int channels) throws ICCProfileException
	{
		...
		if (channels <= 0)
			throw new ICCProfileException("channels <= 0",ICCProfileException.WrongSizeException);
	...
	{
		if (this.measureUnitSignature_==null)
		...
		if (this.patchMeasures_.length != channels || this.responseArray_.length != channels)
			throw new ICCProfileException("not same number of channels",ICCProfileException.InvalidDataValueException);
	...
	public String toXmlString(String name)
	{
		...
		if (name==null || name.length()<1)
			...
			sb.append("<curveStructure name=\""+name+"\">");
